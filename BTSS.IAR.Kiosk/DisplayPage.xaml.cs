...
private async Task<(bool ok, CoreWebView2WebErrorStatus? err)> NavigateWebView2Async(string url, int timeoutMs = 20000)
{
    ...
    public async Task ReloginAsync()
    {
        ...
        if (!string.IsNullOrWhiteSpace(_agency) && !string.IsNullOrWhiteSpace(_username) && !string.IsNullOrWhiteSpace(_password))
            await NavigateAndLoginIfNeededAsync(_initialUrl, _agency, _username, _password);
        else
            await ReloadAsync();
    }

    public async Task ClearSessionAndReloginAsync()
    {
